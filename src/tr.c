	...
	for(i=0; i< giTextNum; i++)
	{
		if(strcmp(str, gpszTrText[i][0])==0)
		{
			if(*(gpszTrText[i][gemLanguage]))
				return gpszTrText[i][gemLanguage];
			else
				return (char*)str;
		}
	}
...
int SetCustomizedFont(EM_LANG emLang)
{
	...
	nRet = NDK_ScrInitTrueTypeFont(szFontName, 0);
	nRet += NDK_PrnInitTrueTypeFont(szFontName, 0);
	nRet += NDK_ScrSetTrueTypeFontEncodingFormat(DISP_TTF_INPUT_UTF8);
	nRet += NDK_PrnSetTrueTypeFontEncodingFormat(DISP_TTF_INPUT_UTF8);
	...
	SetTrLang(emLang);	
	PubSetDispForm(GetVarSystemFontSize(), GetVarSystemFontSize() / 4);
	return APP_SUCC;
}
